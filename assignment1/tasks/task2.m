...
figure;
sgtitle('Golden Section: Function evaluations vs l', 'FontWeight','bold');
for i = 1:3
...
figure;
sgtitle('Golden Section: Interval endpoints a_k, b_k vs iteration k for different l', 'FontWeight','bold');
for i = 1:3
    for j = 1:length(l_values)
        l = l_values(j);
        [~, ~, k, ~, history] = golden_section(funcs{i}, a0, b0, l);
