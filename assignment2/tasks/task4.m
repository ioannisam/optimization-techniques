...
for i = 1:npoints
    ...
    for j = 1:nmethods
        row = row + 1;
        method = methods{j};
        [xk, k, evals, history] = levenberg_marquardt(f_wr, gf_wr, hf_wr, x0, 1e-3, gamma_fixed, e, method);
...
figure;
sgtitle('Levenberg-Marquardt: Convergence history vs iteration k', 'FontWeight','bold');
for i = 1:npoints
    for j = 1:nmethods
        idx = (i-1)*nmethods + j;
